...
- (void)viewDidLoad {
    ...
    [self fetchSellerAddresses];
	
}

- (void)fetchSellerAddresses {
    [self displayHUD:@"加载中..."];
    [[MLAPIClient shared] fetchBussinessInfoForAfterSales:self.afterSalesGoods withBlock:^(NSDictionary *attributes, MLResponse *response) {
        [self displayResponseMessage:response];
        if (response.success) {
            self.sellerInfo = [[MLSellerInfo alloc] init];
            self.sellerInfo.name = [response.data objectForKey:@"name"];
            self.sellerInfo.phone = [response.data objectForKey:@"tel"];
            self.sellerInfo.address = [response.data objectForKey:@"address"];
...
{
    UIEdgeInsets edgeInsets = UIEdgeInsetsMake(10+44+20, 15, 10, 15);
    ...
    _sellerAddressLabel.textColor = [UIColor blackColor];
    _sellerAddressLabel.numberOfLines = 2;
    _sellerAddressLabel.text = self.sellerInfo.address;
    ...
    buyerPhoneLabel.textColor = [UIColor fontGrayColor];
    string = @"*发货人手机号";
    attributedString = [[NSMutableAttributedString alloc] initWithString:string];
    [attributedString addAttributes:attributes range:range];
    ...
    logisticCompanyLabel.textColor = [UIColor fontGrayColor];
    ...
    remarkLabel.textColor = [UIColor fontGrayColor];
    remarkLabel.text = @"备注";
